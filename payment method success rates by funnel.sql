...
-- ============================================================
-- Output: 4 rows (periods) × 12 metrics (4 funnels × 3 payment methods)
...
--       and may succeed in subsequent days. Use for anomaly detection, not final rates.
--
-- All funnels: order-level dedup (MAX), first-transaction date for period attribution
-- ============================================================
...
buy_sub_raw AS (
  ...
  WHERE pt.TransactionType=0 AND pt.Sum>0
    AND DATE(TIMESTAMP(pt.TransactionTime),'Asia/Jerusalem')
...
buy_sub_tagged AS (
  ...
  JOIN periods p ON t.first_date BETWEEN p.d_start AND p.d_end
  WHERE DATE(TIMESTAMP(TransactionTime),'Asia/Jerusalem') < CURRENT_DATE('Asia/Jerusalem')
),
buy_sub_order AS (
